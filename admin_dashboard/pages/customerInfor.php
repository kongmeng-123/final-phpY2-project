...
<body id="page-top">
        ...
        <ul class="navbar-nav accordion bg-gradient-primary sidebar sidebar-dark" id="accordionSidebar">
            ...
            <li class="nav-item active">
                ...
                <div class="collapse" id="collapseCustomer">
                    <div class="py-2 bg-white collapse-inner rounded">
                        <a class="collapse-item active" href="customerInfor.php">User Information</a>
                    </div>
                </div>
            </li>
            <li class="nav-item">
        ...
        <div class="d-flex flex-column" id="content-wrapper">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-2 text-gray-800">Customer's information details</h1>
    ...
    <script>
        ...
        let users = []; // เก็บข้อมูลทั้งหมดไว้ที่นี่

        async function loadProduct() {
            try {
                const response = await fetch('http://localhost:9090/api/api.php/users');
                if (!response.ok) throw new Error("network response was not ok");

                users = await response.json(); // เก็บข้อมูลใส่ตัวแปร Global
                console.log(users)
                renderTable(users); // แสดงผลครั้งแรก
            } catch (error) {
        ...
        document.getElementById("search_data").addEventListener("input", function() {
            ...
            let filtered = users.filter(item => {
                let matchesFullname = (item.Fname +" "+ item.Lname).toLowerCase().includes(filter)
                let matchesId = item.user_id.toString().toLowerCase().includes(filter);
                let matchesFname = item.Fname.toLowerCase().includes(filter);
                let matchesLname = item.Lname.toLowerCase().includes(filter);
                let matchesPhone = item.phoneNumber.toLowerCase().includes(filter);
                let matchesSearch = matchesId || matchesFname || matchesLname || matchesPhone || matchesFullname;
