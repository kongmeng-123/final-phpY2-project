   ...
   switch ($resource) {
         ...
         if ($method === "GET") {
            if ($id) {
               ...
               foreach ($order_data as &$row) {
                  ...
                  $row['order_status'] = $row['status'] ?? 'Pending';
               }
            ...
            $allowedFields = ['product_name', 'product_img_src', 'status', 'product_price', 'amount_product', 'user_name', 'bill_status', 'bill_img_src', 'date_order', 'date_success', 'user_address', 'express_with'];
            $fieldsToUpdate = [];
            $params = [];
            foreach ($allowedFields as $field) {
               if (array_key_exists($field, $input)) {
                  $fieldsToUpdate[] = "`$field` = :$field";
                  $params[$field] = $input[$field];
               } elseif ($field === 'status' && array_key_exists('order_status', $input)) {
                  $fieldsToUpdate[] = "`status` = :status";
                  $params['status'] = $input['order_status'];
               }
            }
      ...
      case "promotions":
         if ($method === "GET") {
            ...
            $stmt = $pdo->prepare("INSERT INTO promotion_tb (title, discount, date_order, update_date, end_date) VALUES (:title, :discount, :date_order, :update_date, :end_date)");
            $stmt->execute([
            ...
            $allowedFields = ['title', 'discount', 'date_order', 'update_date', 'end_date'];
            $fieldsToUpdate = [];
            $params = [];
            foreach ($allowedFields as $field) {
               if (array_key_exists($field, $input)) {
